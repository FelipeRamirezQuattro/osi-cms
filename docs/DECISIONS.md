...
- **Phase 10 (AR Model Block) — replaces the client's Sketchfab/Worldcast.io workflow with a native `ar_model` CMS block**, not a product-only feature. The client's process today is Blender → `.fbx` → Sketchfab (hosting + color edits) → Worldcast.io (generates the WebAR experience + QR). `@google/model-viewer` covers the Worldcast step natively (glTF rendering plus automatic iOS Quick Look / Android Scene Viewer AR launch from one `<model-viewer>` element), so only Sketchfab's two jobs — hosting and color editing — needed a decision. Registered as a general block (`lib/blocks/registry.ts`, standard `ar-model.tsx`/`ar-model-client.tsx` split for the `"use client"` boundary) rather than tied to `products`, so it can be placed on any page via `page_blocks`, not just product detail.
- **Phase 10 — no server-side `.fbx` conversion; the client exports `.glb` and `.usdz` directly from Blender.** A server-side FBX→glTF/USDZ pipeline would need a heavy runtime dependency (headless Blender or an FBX2glTF binary) with no equivalent already in this stack, well outside "minimal footprint." Blender's built-in glTF exporter replaces the Sketchfab upload step; both `.glb` and `.usdz` are required fields on the block (not optional) so AR behaves identically on iOS and Android for every published model — the client's export workflow gains one extra step (a USDZ export/conversion) in exchange for a guaranteed AR experience.
- **Phase 10 — no in-CMS material/color editing; Blender stays the source of truth for how a model looks.** This was Sketchfab's other job besides hosting. Replicating it would mean building a material-override UI against model-viewer's scene-graph API and only works if models are authored for it — real scope, and out of proportion to a first version whose job is just "render the model and let it launch AR." Revisit as a follow-on if the client finds re-exporting from Blender for color tweaks too slow.
- **Phase 10 — the AR block's QR code is generated client-side from `window.location.href`, not server-rendered from a stored URL.** Threading the current page's canonical URL down through `BlockRenderer` into a block's `Render` component isn't a pattern any other block needs; generating it in the browser at render time is simpler, needs no new plumbing, and is correct in every context the block can appear in (public page, `/preview/*`) since it just encodes whatever URL is actually being viewed. Uses a small `qrcode` npm package (SVG output) — no external QR service.
- **Phase 10 — `.glb`/`.usdz` uploads reuse the existing `media` Storage bucket and `lib/validation/media.ts` chokepoint**, adding a third `MediaKind` ("model") alongside "image"/"document", rather than a separate bucket or upload path. Keeps one enforcement point for every upload in the app (Task 11's rationale) and one media library UI, at the cost of that UI needing a generic icon placeholder for kinds it can't thumbnail.

- **Phase 9 follow-up — a staff "Add subscriber" path was added deliberately after shipping signup-only.** The original scope call ("the client asked for the public signup form as the only intake") turned out to have a real gap in practice: there was no way to bring in someone who consented off-site (phone, in person, a trade show) or an existing list once one exists. `addOrResubscribeByStaff()` in `lib/data/newsletter-subscribers.ts` creates (or moves back to) `subscribed` immediately, `source: "admin"`, no confirmation email — it deliberately bypasses double opt-in because a staff member is attesting consent they already have, not collecting it. Gated by `manage_newsletter` (day-to-day, same as tag management), not `delete_content` — adding someone isn't destructive. CSV *import* (bringing in a whole existing list at once) is still out of scope; this only covers one address at a time.
