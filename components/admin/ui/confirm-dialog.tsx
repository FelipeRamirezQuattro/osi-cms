    ...
    >
      {pending && (
        <div className="space-y-4 p-5">
          <h2 className="font-display text-sm tracking-wide-display uppercase">{pending.options.title}</h2>

          {pending.options.message && <p className="text-sm opacity-80">{pending.options.message}</p>}
          ...
          {pending.kind === "prompt" && (
            <label className="block space-y-1 text-sm">
              ...
              />
              {inputError && <span className="block text-xs text-red-600">{inputError}</span>}
            </label>
          )}
